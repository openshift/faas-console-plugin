...
- GCP_PROJECT: `itpc-gcp-hcm-pe-eng-claude`
- GCP_REGION: `us-east5`
...
If no active account, tell the user to run `! gcloud auth login` (the `!` prefix runs it in this session so the browser auth flow works). Wait for them to confirm before continuing.

### 1.5 GCP Application Default Credentials

Check if `$HOME/.config/gcloud/application_default_credentials.json` exists. If missing, tell the user to run `! gcloud auth application-default login`. This is a separate auth step from `gcloud auth login`. The ADC file is what the Java SDK uses and gets mounted as a secret on the cluster.
...
- If not logged in, ask the user for their cluster API URL and tell them to run `! oc login <url>` themselves (interactive login needs their terminal). Wait for them to confirm before continuing.
