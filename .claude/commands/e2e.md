...
1. **Learn the patterns** -- read these files to internalize the project's e2e conventions:
   ...
   - `e2e/listing/function-list-basic.test.ts` (reference test file)
...
4. **Scaffold the test file** at `e2e/<feature>/$ARGUMENTS.test.ts` following these conventions.
   ...
   **Navigation and dialog handling:**
   - Use helpers: `navigateToFunctionsList(page)`, `navigateToFunctionsListWithRealPat(page, pat)`, `navigateToCreatePage(page, pat)`
