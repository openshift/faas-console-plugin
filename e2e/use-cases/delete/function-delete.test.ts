...
test.describe('Delete function', () => {
  ...
  test('shows an enabled deploy button for not deployed functions', async ({ page }) => {
    ...
    await expect(row.getByRole('button', { name: 'Deploy' })).toBeEnabled();
    await expect(row.getByRole('button', { name: 'Undeploy' })).toHaveCount(0);
  });
  ...
  test('undeploy button removes function from cluster', async ({ page }) => {
    ...
    await test.step('undeploy function', async () => {
      ...
      const undeployButton = row.getByRole('button', { name: 'Undeploy' });
      await expect(undeployButton).toBeEnabled({ timeout: 30_000 });
